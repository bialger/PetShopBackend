...
/**
 * Handler for retrieving pets by owner ID
 * GET /api/pets/owner/{ownerId}
 */
@Component
public class GetPetsByOwnerIdHandler implements PetMappingHandler {

    private static final Logger log = LoggerFactory.getLogger(GetPetsByOwnerIdHandler.class);

    private final String method = "GET";
    private final String uriRegex = "^/owner/\\d+$"; // /owner/123

    private final PetService petService;
    private final PetModelToDtoMapper petModelToDtoMapper;

    @Autowired
    public GetPetsByOwnerIdHandler(PetService petService, PetModelToDtoMapper petModelToDtoMapper) {
    ...
    public Optional<Object> handleRequest(GatewayRequest request) {
        ...
        Long ownerId = Long.parseLong(path.substring("/owner/".length()));

        log.info("Processing request to get pets by owner ID: {}", ownerId);

        PageRequest pageRequest = extractPageRequest(request.getQueryParams());
        Page<PetModel> petModelPage = petService.findPetsByOwnerId(ownerId, pageRequest);
